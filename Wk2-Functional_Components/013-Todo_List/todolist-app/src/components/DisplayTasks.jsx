...
const DisplayTasks = ({ todoList, setTodoList }) => {
    ...
    const toggleTaskCompletion = ( taskToUpdate ) => {
        ...
        localStorage.setItem('todoList', JSON.stringify(updatedTodoList)); // Update local storage
                ...
                todoList.map(( task ) => (
                    <div key={ task.id } className='d-flex align-items-center justify-content-between mb-3'>
                        <div className="d-flex align-items-center gap-3">
                            <input type="checkbox" onChange={ ()=>toggleTaskCompletion(task) } checked={ task.completed } className='form-check-input border border-secondary' />
